...
class CartRewardBloc extends Bloc<CartRewardEvent, CartRewardState> {
  ...
  CartRewardBloc({required this.cartDatasource, required this.productDatasource}) : super(const CartRewardState.initial()) {
    on<_GetCart>((event, emit) async {
      ...
      await result.fold((l) {
        ...
        if (cartRewardModel.cart?.cartItems != null) {
          ...
          emit(_Success(cartModel: cartRewardModel, productDetails: productDetails, patchQtyModel: null, deleteModel: null));
        } else {
